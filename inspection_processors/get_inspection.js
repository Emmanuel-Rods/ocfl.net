 ...
 * @param {string} ProcessRSN - inspection number
 * @returns {Promise<string>} - The HTML response from the server.
 */

async function getInspection(payload, ProcessRSN) {
  if (!ProcessRSN) {
    throw new Error("ProcessRSN (Inspection number) is missing");
  }

  const url = "https://fasttrack.ocfl.net/OnlineServices/ProcessDetail.aspx";
